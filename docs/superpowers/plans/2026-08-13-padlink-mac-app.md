...
- Create: `Padlink/PadlinkMac/Info.plist`
...
- [ ] **Step 1: Ignore the generated project**
...
- [ ] **Step 3: Write the app and its Info.plist**

`Padlink/PadlinkMac/Info.plist` can be minimal, because `project.yml` supplies the properties:
...
**Task 0's spike decides one thing here:** whether `kSecUseDataProtectionKeychain: true` is set. Use whatever the spike measured. Everything else in this task is unchanged either way.
...
final class KeychainPairingStore: PairingStore {
    ...
    private func baseQuery(account: String?) -> [String: Any] {
        var query: [String: Any] = [
            ...
            // Set per the Task 0 spike result.
...
If these fail with a Keychain status error, apply the Task 0 spike's answer: remove the `kSecUseDataProtectionKeychain` line to use the legacy keychain. Do not guess; use what the spike measured.
